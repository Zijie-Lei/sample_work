...
(function (DnatcoUtil) {
    ...
    function getAtomIndex(loc, residue, names, altId, insCode) {
        for (var eI = residue.start; eI < residue.end; eI++) {
            ...
            var elInsCode = structure_1.StructureProperties.residue.pdbx_PDB_ins_code(loc);
    ...
    function getStepIndices(data, loc, r) {
        ...
        var modelIdx = structure_1.StructureProperties.unit.model_num(loc) - 1;
        var chainId = structure_1.StructureProperties.chain.auth_asym_id(loc);
        var seqId = structure_1.StructureProperties.residue.auth_seq_id(loc);
        var insCode = structure_1.StructureProperties.residue.pdbx_PDB_ins_code(loc);
    ...
    function residueAltIds(structure, unit, residue) {
        ...
        var loc = structure_1.StructureElement.Location.create(structure, unit);
        for (var eI = residue.start; eI < residue.end; eI++) {
    ...
    var _loc = structure_1.StructureElement.Location.create();
    function residueToLoci(asymId, seqId, altId, insCode, loci, source) {
        _loc.structure = loci.structure;
        var _loop_1 = function (e) {
            _loc.unit = e.unit;
            var getAsymId = source === 'label' ? structure_1.StructureProperties.chain.label_asym_id : structure_1.StructureProperties.chain.auth_asym_id;
            var getSeqId = source === 'label' ? structure_1.StructureProperties.residue.label_seq_id : structure_1.StructureProperties.residue.auth_seq_id;
            // Walk the entire unit and look for the requested residue
            var chainIt = int_1.Segmentation.transientSegments(e.unit.model.atomicHierarchy.chainAtomSegments, e.unit.elements);
            var residueIt = int_1.Segmentation.transientSegments(e.unit.model.atomicHierarchy.residueAtomSegments, e.unit.elements);
            var elemIndex = function (idx) { return int_1.OrderedSet.getAt(e.unit.elements, idx); };
            while (chainIt.hasNext) {
                ...
                while (residueIt.hasNext) {
                    ...
                    if (_seqId === seqId) {
                        var _insCode = structure_1.StructureProperties.residue.pdbx_PDB_ins_code(_loc);
                        if (_insCode !== insCode)
                            continue;
                        if (altId) {
                        ...
                        var end = residue.end;
                        return { value: structure_1.StructureElement.Loci(loci.structure, [{ unit: e.unit, indices: int_1.OrderedSet.ofBounds(start, end) }]) };
                    }
                }
            }
        };
...
})(DnatcoUtil = exports.DnatcoUtil || (exports.DnatcoUtil = {}));
